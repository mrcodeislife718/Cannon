const KEYWORDS = new Set(['fn','return','if','else','while','true','false','null','let','const']);
  ...
  while (i < source.length) {
    ...
    if (ch === '"' || ch === "'") {
      ...
      while (i < source.length && current() !== quote) {
        if (current() === '\\') {
          advance();
          if (i >= source.length) throw new CannonSyntaxError('Unterminated string escape', startLine, startColumn);
          const esc = advance();
          const escapes = { n:'\n', r:'\r', t:'\t', '"':'"', "'":"'", '\\':'\\' };
    ...
    if (/\d/.test(ch)) {
      let raw = '';
      while (i < source.length && /\d/.test(current())) raw += advance();
      if (current() === '.' && /\d/.test(source[i + 1] ?? '')) {
        raw += advance();
        while (i < source.length && /\d/.test(current())) raw += advance();
      }
    ...
    if (/[A-Za-z_]/.test(ch)) {
      ...
      push(KEYWORDS.has(value) ? value : 'identifier', value, startLine, startColumn);
      continue;
    }
    ...
    if (['==','!=','<=','>=','&&','||'].includes(two)) {
      advance(); advance();
      push(two, two, startLine, startColumn);
      continue;
    }
